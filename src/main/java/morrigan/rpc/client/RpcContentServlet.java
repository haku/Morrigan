...
import mediatoad.rpc.MediaToadProto.ReadMediaRequest.Builder;
...
public class RpcContentServlet implements ContentServer {
	...
	@Override
	public void doHead(HttpServletRequest req, HttpServletResponse resp, String listId, String itemId) throws IOException {
		...
		resp.setContentLengthLong(hasMedia.getItem().getFileLength());
		resp.setContentType(hasMedia.getItem().getMimeType());
	}
	...
	public void doGet(final HttpServletRequest req, final HttpServletResponse resp, final String listId, final String itemId) throws IOException {
		...
		try (final CancellableContext context = Context.current().withCancellation()) {
			...
			try {
				...
				if (!resp.isCommitted()) {
					resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "RPC error: " + e.getStatus());
				}
				context.cancel(e);
			}
			finally {
