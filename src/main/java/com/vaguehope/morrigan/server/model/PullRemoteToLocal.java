...
public class PullRemoteToLocal implements MorriganTask {
	...
	private final MediaDb ldb;
	private final URI remoteUri;
	private final MediaFactory mediaFactory;
	private final Config config;

	public PullRemoteToLocal (final MediaDb localDb, final URI remoteUri, final MediaFactory mediaFactory, final Config config) {
		this.ldb = localDb;
	...
	public TaskResult run (final TaskEventListener extTaskEventListener) {
		...
		try {
			...
			final String file = RemoteMediaDbHelper.getFullPathToMmdb(this.config, uuid.toString());
			final RemoteHostDetails details = new RemoteHostDetails(this.remoteUri);
			final RemoteMediaDb rdb = RemoteMediaDbFactory.getNew(file, details);
